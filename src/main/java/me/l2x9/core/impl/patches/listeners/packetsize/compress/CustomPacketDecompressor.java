...
public class CustomPacketDecompressor extends ByteToMessageDecoder {
    ...
    public CustomPacketDecompressor(int threshold) {
    ...
    public void decode(ChannelHandlerContext ctx, ByteBuf buf, List<Object> list) throws Exception {
        if (buf.readableBytes() != 0) {
            ...
            if (totalLength >= 1927951) {
                pds.clear();
                Utils.log(String.format("&aPrevented an extremely large packet with length of&r&3 %d/%d &r&afrom being received&r", totalLength, 1927951));
                return;
            }
            if (totalLength == 0) {
